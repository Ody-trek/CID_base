...
class CIDHead(nn.Module):
    ...
    def __init__(self,
        ...
        # 将gfd模块输出特征图channel变为17
        self.conv_17 = nn.Conv2d(in_channels=32, out_channels=17, kernel_size=1, stride=1)

        # 添加Linear Projection 将17个channel中每个channel的2D matrix拉成1D vector
        cfg = Config.fromfile('/home/chou/deep/mmpose/configs/body/2d_kpt_sview_rgb_img/cid/coco/hrnet_w32_coco_512x512.py')
        print('---cfg.SIMDR_SPLIT_RATIO---：', cfg.SIMDR_SPLIT_RATIO)
        print('---cfg.data_cfg.image_size---:', cfg.data_cfg.image_size)
        self.mlp_head_x = nn.Linear(cfg.HEAD_INPUT, int(cfg.data_cfg.image_size * cfg.SIMDR_SPLIT_RATIO))
        self.mlp_head_y = nn.Linear(cfg.HEAD_INPUT, int(cfg.data_cfg.image_size * cfg.SIMDR_SPLIT_RATIO))
    ...
    def forward(self, features, forward_info=None):
        ...
        # print('---features shape (传入forward_test)---: ', features.shape)                 # 自己加
        print('---forward_info.shape---: ', len(forward_info))
        # print('---forward_info---: ', forward_info)                                       # 自己加
        if self.training:
    ...
    def forward_train(self, features, labels):
        ...
        print('---gt_instance_coord.shape---: ', gt_multi_heatmap.shape)
        # print('---gt_instance_coord---: ', gt_multi_heatmap)
        print('---gt_instance_heatmap.shape---: ', gt_instance_heatmap.shape)
        # print('---gt_instance_heatmap---: ', gt_instance_heatmap)
        print('---gt_instance_mask.shape---: ', gt_instance_mask.shape)
        # print('---gt_instance_mask---: ', gt_instance_mask)
        ...
        cond_instance_feats = torch.cat((c_instance_feats, s_instance_feats),
                                        dim=1)
        print('===c_instance_feats.shape===: ', c_instance_feats.shape)
        print('===s_instance_feats.shape===: ', s_instance_feats.shape)
        print('===1_cond_instance_feats.shape===: ', cond_instance_feats.shape)
        cond_instance_feats = self.fuse_attn(cond_instance_feats)
        print('===2_cond_instance_feats.shape===: ', cond_instance_feats.shape)
        print('===2_cond_instance_feats===: ', cond_instance_feats)


        cond_instance_feats = F.relu(cond_instance_feats)
        print('===cond_instance_feats.shape (after relu)===: ', cond_instance_feats.shape)
        channel_17 = self.conv_17(cond_instance_feats)              # channel：32 -> 17
        print('---channel_17.shape---:', channel_17.shape)
        channel_17 = rearrange(channel_17, 'b c h w -> b c (h w)')  # 将2D matrix拉成1D向量（128*128 = 16,384维度）
        print('---channel_17.shape---:', channel_17.shape)


        pred_x = self.mlp_head_x(channel_17)
        pred_y = self.mlp_head_y(channel_17)
    ...
    def forward_test(self, features, test_cfg):
        # print('---features shape---', features.shape)                # 自己加
        flip_test = test_cfg.get('flip_test', False)
        # print('---flip_test---', flip_test)                    # 自己加
        ...
        # flip back feature map   这个flip什么意思？
        if flip_test:
        ...
        s_instance_feats = self.s_attn(instance_features, instance_params,
                                       instances['instance_coord'])
        # print('---c_instance_feats shape---: ', c_instance_feats.shape)           # 自己加
        # print('---s_instance_feats shape---: ', s_instance_feats.shape)           # 自己加
        # print('---c_instance_feats和s_instance_feats的前两个维度含义？---')
        ...
        # print('---instance_heatmaps shape---: ', instance_heatmaps.shape)
        ...
        # print('---instance_heatmaps (after if flip_test) shape---: ', instance_heatmaps.shape)           # 自己加
        # print('---scores---: ', scores, '---scores shape---: ',scores.shape)                             # 自己加
        # print('---scores 是什么？---')                                                                     # 自己加
