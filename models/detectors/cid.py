...
class CID(BasePose):
    ...
    def forward_train(self, img, multi_heatmap, multi_mask, instance_coord,
        ...
        print('===len(losses) (from cid.py)===:', len(losses))
        print('===losses (from cid.py)===: ', losses)
        return losses
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        # print('--image_resized shape (before concatenate)--', image_resized.shape)
        image_resized_before = image_resized
        if self.test_cfg.get('flip_test', True):
            image_flipped = torch.flip(image_resized, [3])
            image_resized = torch.cat((image_resized, image_flipped), dim=0)
            # print('if image_resized_before is equal to image_flipped: ', image_resized_before.equal(image_flipped))
            # print('if image_resized_before is eq to image_flipped: ', image_resized_before.eq(image_flipped))
            # print('---image_resized---： ', image_resized)
            # print('是这里resize成512x512输入到CID模型？看样子不是...(test时只将短边resize为512) ---image_resized shape---： ', image_resized.shape)
        features = self.backbone(image_resized)
        instance_heatmaps, instance_scores = self.keypoint_head(
            features, self.test_cfg)
        # print('---instance_heatmaps shape:---', instance_heatmaps.shape)            # 自己加
        # print('---instance_scores shape:---', instance_scores.shape)                # 自己加
        ...
        # print('---result type:---', type(result))                                 # 自己加
        # print('---result:---', result.keys())                                     # 自己加
        # print('---result[\'preds\'] shape:---', torch.tensor(result['preds']).shape)         # 自己加
        # print('---result---: ', result)
        return result
